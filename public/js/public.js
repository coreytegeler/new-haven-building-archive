...
(function() {
  window.initPublic = function() {
    var $body, $buildingTiles, $buildings, $filterSect, $glossarySect, $grid, $gridWrap, $infoSect, $main, $searchSect, $side, $singleSect, buildingMapSetup, centerGrid, clearFilter, clickBuilding, clickFilter, clickMarker, clickToggle, closeSide, filter, filterIsOn, filterQuery, filterUrl, getContent, getParam, getParams, hoverBuilding, infoMapSetup, insertMap, makeDraggable, nextSlide, openSide, paginate, popState, resizeGrid, selectBuilding, setUp, setUpSlider, tourMapSetup, unhoverBuilding, updateSingleSect, urlQuery;
    $body = $('body');
    $main = $('main');
    $side = $('aside');
    $gridWrap = $('.gridWrap');
      ...
      return geocoder.geocode({
        ...
        if (status === 'OK') {
          ...
          $(allBuildings).each(function(i, building) {
            return geocoder.geocode({
              'address': building.address + ', New Haven, CT 06510'
            }, function(results, status) {
              var marker;
              if (status === 'OK') {
                coords = results[0].geometry.location;
    ...
    tourMapSetup = function() {
      var $map, $mapWrap, $section, bounds, color, geocoder, mapObj;
      geocoder = new google.maps.Geocoder();
      ...
      bounds = new google.maps.LatLngBounds();
      return $(buildingsInTour).each(function(i, building) {
        geocoder.geocode({
          'address': building.address + ', New Haven, CT 06510'
        }, function(results, status) {
          var coords, marker;
